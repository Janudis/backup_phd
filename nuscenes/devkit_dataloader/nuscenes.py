...
from lidarseg.lidarseg_utils import colormap_to_colors, plt_to_cv2, get_stats, \
    get_labels_in_coloring, create_lidarseg_legend, paint_points_label
from panoptic.panoptic_utils import paint_panop_points_label, stuff_cat_ids, get_frame_panoptic_instances,\
    get_panoptic_instances_stats
from utils.data_classes import LidarPointCloud, RadarPointCloud, Box
from utils.data_io import load_bin_file, panoptic_to_lidarseg
...
class NuScenes:
    ...
    def __init__(self,
                ...
                #  dataroot: str = '/media/dimitris/4b3f6643-e758-40b9-9b58-9e98f88e5c791/dimitris/data/sets/nuscenes',
                 verbose: bool = True,
...
class NuScenesExplorer:
    ...
    #     pc1.rotate(Quaternion(cs_record['rotation']).rotation_matrix)
    #     pc1.translate(np.array(cs_record['translation']))
    #     lidar2 = pc1.points[:3, :]
    #     pc2 = copy.deepcopy(pc1)
    #     # Second step: transform from ego to the global frame.
    #     poserecord = self.nusc.get('ego_pose', pointsensor['ego_pose_token'])
    #     pc2.rotate(Quaternion(poserecord['rotation']).rotation_matrix)
    #     pc2.translate(np.array(poserecord['translation']))
    #     lidar3 = pc2.points[:3, :]
    #     pc3 = copy.deepcopy(pc2)
    #     # Third step: transform from global into the ego vehicle frame for the timestamp of the image.
    #     poserecord = self.nusc.get('ego_pose', cam['ego_pose_token'])
    #     pc3.translate(-np.array(poserecord['translation']))
    #     pc3.rotate(Quaternion(poserecord['rotation']).rotation_matrix.T)
    #     lidar4 = pc3.points[:3, :]
    #     pc4 = copy.deepcopy(pc3)
    #     # Fourth step: transform from ego into the camera.
    #     cs_record = self.nusc.get('calibrated_sensor', cam['calibrated_sensor_token'])
    #     pc4.translate(-np.array(cs_record['translation']))
    #     pc4.rotate(Quaternion(cs_record['rotation']).rotation_matrix.T)
    #     lidar5 = pc4.points[:3, :]
    ...
    #     return points, coloring, im, lidar1, lidar2, lidar3, lidar4, lidar5
    ...
    def map_pointcloud_to_image(self,
        ...
        # print(f"lidar pc: {lidar_points}")
        im = Image.open(osp.join(self.nusc.dataroot, cam['filename']))
        print(f"original image size (nuscenes.py): {im.size}")
        #img = self.transform(im)
        new_size = (512, 512)
        # img = im
        img = im.resize(new_size)
        #print(img.size)
        # print(f"first pc: {pc.points}")
        # Points live in the point sensor frame. So they need to be transformed via global to the image plane.
        # First step: transform the pointcloud to the ego vehicle frame for the timestamp of the sweep.
        cs_record = self.nusc.get('calibrated_sensor', pointsensor['calibrated_sensor_token'])
        #print(cs_record)
        ...
        # print(f"2 pc: {pc.points}")
        # Second step: transform from ego to the global frame.
        poserecord = self.nusc.get('ego_pose', pointsensor['ego_pose_token'])
        pc.rotate(Quaternion(poserecord['rotation']).rotation_matrix)
        pc.translate(np.array(poserecord['translation']))
        # print(f"3 pc: {pc.points}")
        # Third step: transform from global into the ego vehicle frame for the timestamp of the image.
        poserecord = self.nusc.get('ego_pose', cam['ego_pose_token'])
        pc.translate(-np.array(poserecord['translation']))
        pc.rotate(Quaternion(poserecord['rotation']).rotation_matrix.T)
        # print(f"4 pc: {pc.points}")
        # Fourth step: transform from ego into the camera.
        cs_record = self.nusc.get('calibrated_sensor', cam['calibrated_sensor_token'])
        pc.translate(-np.array(cs_record['translation']))
        pc.rotate(Quaternion(cs_record['rotation']).rotation_matrix.T)
        # print(f"rotation: {Quaternion(cs_record['rotation']).rotation_matrix.T}")
        # print(f"5 pc: {pc.points}")
        #pc.scale(scaling_matrix)
        # print(f"camera frame: {pc.points}")
        # Fifth step: actually take a "picture" of the point cloud.
        # Grab the depths (camera frame z axis points away from the camera).
        depths = pc.points[2, :]
        ...
        # points = view_points(pc.points[:3, :], np.array(cs_record['camera_intrinsic']), normalize=True)
        # print(pc.points)
        # points = np.dot(scaling_matrix, points[:3, :]) #added by me because i resize the image
        # print(pc.points)
        """
        Since i resize the image, the more correct approach would be to change the camera_intrinsic. 
        Below the code for it. Notice that the result is not the same but very similar.
        """
        old_width, old_height = 1600, 900  # Original dimensions
        new_width, new_height = 512, 512   # New dimensions
        # Convert the list to a numpy array
        camera_intrinsics_array = np.array(cs_record['camera_intrinsic'])
        # print(f"points {pc.points[:3, :]}")
        # Extracting the original intrinsic parameters
        f_x = camera_intrinsics_array[0, 0]
        f_y = camera_intrinsics_array[1, 1]
        c_x = camera_intrinsics_array[0, 2]
        c_y = camera_intrinsics_array[1, 2]

        keep_aspect_ratio = False
        if keep_aspect_ratio:
            # Adjusting the intrinsic parameters for the resized image
            f_x_new = f_x * new_height / old_height
            f_y_new = f_y * new_height / old_height
            c_x_new = c_x * new_height / old_height
            c_y_new = c_y * new_height / old_height

        else:
            # Adjusting the intrinsic parameters for the resized image
            f_x_new = f_x * new_width / old_width
            f_y_new = f_y * new_height / old_height
            c_x_new = c_x * new_width / old_width
            c_y_new = c_y * new_height / old_height

        # Updating the camera intrinsic matrix in the numpy array
        camera_intrinsics_array[0, 0] = f_x_new
        camera_intrinsics_array[1, 1] = f_y_new
        camera_intrinsics_array[0, 2] = c_x_new
        camera_intrinsics_array[1, 2] = c_y_new

        visual_Det_camera_intrinsic = np.array([[810.50700995,   0, 522.41089264,   0],
        [0, 810.50700995, 250.56452211,   0],
        [0,   0,   1,   0]])

        # print(f"intrinsic {camera_intrinsics_array}")
        points = view_points(pc.points[:3, :], camera_intrinsics_array, normalize=True)
        # print(f"points after {points}")
        # Remove points that are either outside or behind the camera. Leave a margin of 1 pixel for aesthetic reasons.
        # Also make sure points are at least 1m in front of the camera to avoid seeing the lidar points on the camera
        # casing for non-keyframes which are slightly out of sync.
        print(f"lidar_pc {pc.points[:3, :]}")
        print(f"img {img}")
        print(f"camera intrinsic {camera_intrinsics_array}")
        ...
        masked_pc = pc.points[:3, :][:, mask]
        print(f"points after {points}")
        """
        points = 2d projected points [x,y,1]
        coloring = color of each point based on the depth
        img = image
        pc.points[:3, :] = lidar pc before projection
        original_points_leading_to_points = lidar pc after masking = masked_pc
        """
        # print(f"points {points}")
        # print(f"pc.points[:3, :] {pc.points[:3, :]}")
        print(f"pc.points {pc.points.shape}")
        return points, coloring, img, pc.points[:3, :], masked_pc
    ...
    def render_pointcloud_in_image(self,
        ...
        points, coloring, im, lidar_points, masked_pc = self.map_pointcloud_to_image(pointsensor_token, camera_token,
                                                            render_intensity=render_intensity,
                                                            show_lidarseg=show_lidarseg,
                                                            filter_lidarseg_labels=filter_lidarseg_labels,
                                                            lidarseg_preds_bin_path=lidarseg_preds_bin_path,
        ...
        if ax is None:
            fig, ax = plt.subplots(1, 1, figsize=(9, 16))
            if lidarseg_preds_bin_path:
                plt.figure(sample_token + '(predictions)')
            else:
                plt.figure(sample_token)
        else:  # Set title on if rendering as part of render_sample.
            ax.set_title(camera_channel)
        ax.imshow(im)
