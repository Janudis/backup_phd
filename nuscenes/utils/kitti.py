...
class KittiDB:
    ...
    @staticmethod
    def calculate_alpha(box_center_x, p_left_kitti, box_yaw):
        # Camera parameters
        f_u = p_left_kitti[0, 0]  # Focal length in width
        c_u = p_left_kitti[0, 2]  # Principal point in width

        # Compute the observation angle for the 3D bounding box center
        alpha = np.arctan2(box_center_x - c_u, f_u)
        alpha -= box_yaw  # Adjust alpha by the yaw angle

        # Normalize alpha to be within -pi and pi
