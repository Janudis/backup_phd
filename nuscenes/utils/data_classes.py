...
class PointCloud(ABC):
    ...
    def scale(self, scaling_matrix: np.ndarray) -> None:
        """
        Applies a scaling.
        :param scaling_matrix: <np.float: 3, 3>. Scaling matrix.
        """
        #scaling_matrix = np.array([0.32,0,0][0,0.57,0][0,0,1])
        self.points[:3, :] = np.dot(scaling_matrix, self.points[:3, :])
