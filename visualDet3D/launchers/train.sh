...
if [ $NUM_GPUS == 1 ]; then 
    ...
    CUDA_VISIBLE_DEVICES=$GPUS python -m torch.distributed.launch --nproc_per_node=$NUM_GPUS scripts/train.py --config=$CONFIG_PATH --experiment_name=$EXP_NAME --world_size=$NUM_GPUS
fi
