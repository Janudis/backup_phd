...
def get_model_url(name):
    return join(WEB_ROOT, 'imagenet',
                '{}-{}.pth'.format(name, model_hash[name]))
...
class DLA(nn.Module):
    ...
    def __init__(self, levels, channels, num_classes=1000,
                 block=BasicBlock, residual_root=False, out_indices:Tuple[int, ...]=(-1, 0, 1, 2, 3, 4, 5)
                 ):
...
def dlanet(depth, **kwargs):
    if depth == 34:
        ...
        raise ValueError(
            'Unsupported model depth, must be one of 34, 60, 102, 169')
    return model